...

{
    ...
    GST_DEBUG(RED_STR("rc= %d, consumlen=%d \n"), rc, psOVDecObj->byteConsumed);
    ...
    GST_DEBUG(RED_STR("len = %d, samplerate = %d\n",psOVDecObj->outNumSamples, psOVDecObj->SampleRate));
   ...
   // g_print("set time:%lld\n", vordec_info->time_offset);
    ...
    GST_DEBUG(GREEN_STR("gst pad push data: %d\n", GST_BUFFER_SIZE(outbuffer)));
...
mfw_gst_vorbisdec_chain(GstPad * pad, GstBuffer * buf)
{
    ...
    if (!vordec_info->init_done) {
        ...
        if (!GST_CLOCK_TIME_IS_VALID(vordec_info->time_offset)) {
            ...
            GST_DEBUG(GREEN_STR("time_offset = %lld\n", vordec_info->time_offset));
        }
        ...
        if ((inbuffsize = gst_adapter_available(vordec_info->pAdapter))<BS_BUF_SIZE)
            return GST_FLOW_OK;
        GST_DEBUG(RED_STR("inbuffsize: %d\n  ", inbuffsize));

        inbuffer = (guint8*)gst_adapter_peek(vordec_info->pAdapter, inbuffsize);

        GST_DEBUG(RED_STR("inbuffer[0]=0x%x, inbuffer[0]=0x%x \n", inbuffer[0], inbuffer[1]));
        ...
        GST_DEBUG(RED_STR("query_size = %d, psOVDecObj = 0x%x \n", query_size, psOVDecObj));
    ...
    while ((inbuffsize = gst_adapter_available(vordec_info->pAdapter))>(BS_BUF_SIZE)){
        ...
        GST_DEBUG(RED_STR("chain flushlen = %d\n", flushlen));
...
mfw_gst_vorbisdec_sink_event(GstPad * pad, GstEvent * event)
{
    ...
    switch (GST_EVENT_TYPE(event)) {
        case GST_EVENT_NEWSEGMENT:
        {
            ...
            GST_DEBUG(RED_STR("nstart= %ld, nstop = %ld\n ", nstart, nstop));
            ...
            GST_DEBUG(RED_STR("nstart = %ld\n ", nstart));
            GST_DEBUG(RED_STR("nevent = %s\n",nevent));
        ...
        case GST_EVENT_EOS:
        {
            ...
            while ((inbuffsize = gst_adapter_available(vordec_info->pAdapter))>0){
                gint flushlen;
                flushlen = mfw_gst_vorbisdec_data(vordec_info, inbuffsize);
                GST_DEBUG(RED_STR("EOS flushlen = %d\n", flushlen));
                if (flushlen == -1 )
                    break;
...
mfw_gst_vorbisdec_init(MFW_GST_VORBISDEC_INFO_T * vordec_info)
{
    ...
    GST_DEBUG(" \n in mfw_gst_vorbisdec_init routine \n");
    vordec_info->sinkpad =
...
mfw_gst_vorbisdec_base_init(MFW_GST_VORBISDEC_CLASS_T * klass)
{
    ...
    "Freescale vorbis Decoder Plugin",
    "Codec/Decoder/Audio",
    "decodes vorbis raw bitstreams",
...
GST_PLUGIN_DEFINE(GST_VERSION_MAJOR,    /* major version of Gstreamer    */
          ...
          "mfw_vorbisdecoder",  /* name of the plugin            */
          "decodes vorbis streams ",    /* what plugin actually does     */
...
static gboolean plugin_init(GstPlugin * plugin)
{
    return gst_element_register(plugin, "mfw_vorbisdecoder",
                GST_RANK_PRIMARY+1, MFW_GST_TYPE_VORBISDEC);
}
