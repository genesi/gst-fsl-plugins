...
#if !defined(VPU_MX51) && !defined(VPU_MX27) && !defined(VPU_MX53)
#pragma error "Platform defines are not configured correctly - must be MX27, MX51 or MX53"
#endif
					...
					GParamSpec * pspec)
{
    ...
    case MFW_GST_VPUENC_INTRA_QP:
        ...
        GST_DEBUG(">>VPU_ENC: CBR IntraQP Parameter=%d\n", vpu_enc->intra_qp );
        break;
...
gboolean mfw_gst_vpuenc_alloc_framebuffer(MfwGstVPU_Enc * vpu_enc)
{
    ...
    for (i=0;i<framenum;i++) {
        ...
        frame[i].strideY = vpu_enc->width;
        frame[i].strideC = vpu_enc->width / 2;
...
int mfw_gst_vpuenc_fill_headers(MfwGstVPU_Enc *vpu_enc)
{
    ...
    if (vpu_enc->intra_qp != VPU_DEFAULT_QP)
        vpu_EncGiveCommand(vpu_enc->handle, ENC_SET_INTRA_QP, &vpu_enc->intra_qp);
...
#endif
    // Revise for cropping settings.
    if ((vpu_enc->width > vpu_enc->crop_left) &&
        (vpu_enc->height > vpu_enc->crop_top) ) {
        vpu_enc->encOP->picWidth = vpu_enc->width  - vpu_enc->crop_left;
        vpu_enc->encOP->picHeight = vpu_enc->height - vpu_enc->crop_top;
        if (vpu_enc->encOP->picWidth > vpu_enc->crop_right)
            vpu_enc->encOP->picWidth -=  vpu_enc->crop_right;

        if (vpu_enc->encOP->picHeight > vpu_enc->crop_bottom)
            vpu_enc->encOP->picHeight -= vpu_enc->crop_bottom;
        
        
    }
    else {
        GST_ERROR("Crop value is exceed the source value\n");
        vpu_enc->encOP->picWidth = vpu_enc->width;
        vpu_enc->encOP->picHeight = vpu_enc->height;
    }
    GST_INFO("VPU encoder resolution:%d x %d\n",
        vpu_enc->encOP->picWidth, vpu_enc->encOP->picHeight);
...
GstFlowReturn mfw_gst_vpuenc_copy_sink_start_frame(MfwGstVPU_Enc *vpu_enc)
{
    ...
    if (!vpu_enc->bytes_consumed && IS_DMABLE_BUFFER(vpu_enc->gst_buffer)) {
        ...
        g_print (">>VPU_ENC:  Memcpy %d from input \n", to_copy);
...
static void mfw_gst_CalcMJPEGQuantTables(MfwGstVPU_Enc *vpu_enc)
{
    ...
    /* FIXME: The low quality value will cause MJPEG cannot be decoded */
    if (new_quality < 10)
        return;
                                                        ...
                                                        GstStateChange transition)
{
    ...
    switch (transition) 
    {
        ...
        case GST_STATE_CHANGE_READY_TO_PAUSED:
	    {
            ...
            vpu_enc->encParam->enableAutoSkip = 1;
...
#else
                /* Disable the userQpMax and userGamma to avoid mosaic issue */
#if 0
                if (vpu_enc->max_qp != VPU_DEFAULT_QP) 
            ...
            if (mode == STD_MPEG4) {
                ...
                if (vpu_enc->bitrate == 0)
                {
                    ...
                    /* Set with the default MPEG4 QP parameter */
                    vpu_enc->encOP->rcIntraQp = VPU_MAX_MPEG4_QP;
                }
                ...
                if (vpu_enc->bitrate == 0)
                {
                    ...
                    /* Set with the default H264 QP parameter */
                    vpu_enc->encOP->rcIntraQp = VPU_DEFAULT_H264_QP;
                }
#if (defined(VPU_MX51) || defined(VPU_MX53))
...
static void mfw_gst_vpuenc_class_init(MfwGstVPU_EncClass *klass)
{
    ...
    g_object_class_install_property (gobject_class, MFW_GST_VPUENC_INTRA_QP,
        ...
        "QP to use for all I frames - range is 0-51 for H264 and 1-31 for MPEG4 - lower value brings better video quality but higher frame sizes",
        0,51, 0, G_PARAM_READWRITE));
								 ...
								 GstCaps *caps, GstBuffer **buf)
{
    ...
    GST_DEBUG("called vpuenc buffer alloc\n");
   {
    ...
    {
        g_print("\n VPU_ENC: Could not allocate buffer\n");
        *buf = NULL;
        return GST_FLOW_ERROR;
...
static void mfw_gst_vpuenc_init(MfwGstVPU_Enc *vpu_enc, MfwGstVPU_EncClass *gclass)
{
    ...
    vpu_enc->intra_qp      = VPU_DEFAULT_QP;
    vpu_enc->gamma         = 4096;
...
#if (defined(VPU_MX51) || defined(VPU_MX53))        /* what our plugin actually does */
