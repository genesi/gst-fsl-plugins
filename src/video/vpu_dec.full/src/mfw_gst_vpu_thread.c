...
vpu_thread_mutex_lock (MfwGstVPU_Thread * vpu_thread)
{
    ...
    thread_mutex_cnt++;
    //GST_DEBUG(">>VPU_THREAD LOCK mutex_cnt = %d req=%d\n", thread_mutex_cnt, thread_mutex_req);
}
...
vpu_thread_mutex_unlock (MfwGstVPU_Thread * vpu_thread)
{
    ...
    thread_mutex_cnt--;
    //GST_DEBUG(">>VPU_THREAD UNLOCK mutex_cnt = %d req=%d\n", thread_mutex_cnt, thread_mutex_req);
}
...
vpu_dec_thread_decode_completion_wait (MfwGstVPU_Dec * vpu_dec)
{
    ...
    while (fBusy) {
        ...
        if (!vpu_dec->file_play_mode) {
            if (vpu_dec->must_copy_data) {
                vpu_dec->retval =
                    mfw_gst_vpudec_copy_sink_input (vpu_dec,
                                                    vpu_dec->gst_buffer);
    ...
    /* FIXME */
    if ((VPU_PIC_TYPE_IDR) && (vpu_dec->codec != STD_MPEG2) && (vpu_dec->decParam->iframeSearchEnable)) {
        GST_FRAMEDROP("Got I frames, set iframeSearchEnable to FALSE \n");
...
vpu_thread_check_output (MfwGstVPU_Dec * vpu_dec)
{
    ...
    if ((VPU_PIC_TYPE_IDR) && (vpu_dec->decParam->iframeSearchEnable)) {
        GST_FRAMEDROP("Got I frames, set iframeSearchEnable to FALSE \n");
        vpu_dec->decParam->iframeSearchEnable = 0;
    }
    ...
    if (vpu_dec->just_flushed) { // at least decoded or display has to be positive for us to reset iframesearch
        if (VPU_PIC_TYPE_IDR) {
            GST_FRAMEDROP
                (">>VPU_DEC: I frame received so resume play at %d \n",
                 vpu_dec->frames_decoded);
        ...
        if (VPU_PIC_TYPE_IDR) {
            GST_FRAMEDROP
                (">>VPU_DEC: I frame for H.264 now start displaying \n");
            vpu_dec->check_for_iframe = FALSE;
            vpu_dec->check_for_bframe = FALSE;
        }
        else {
            GST_FRAMEDROP
                (">>VPU_DEC: wait to display until next I frame \n");
            mfw_gst_vpudec_no_display (vpu_dec);
            return VPU_DEC_CONTINUE;
        }
    }
    ...
    if (G_UNLIKELY (vpu_dec->outputInfo->indexFrameDisplay < 0)) {
        ...
        if ((vpu_dec->outputInfo->indexFrameDisplay == -1) &&
            ...
            GST_DEBUG
                (">>VPU_DEC: VPU hits EOS must reset on next buffer \n");
            vpu_dec->eos = TRUE;
        }
        if (VPU_PIC_TYPE_IDR)
        ...
        if (vpu_dec->outputInfo->mp4PackedPBframe == 1) {
            GST_FRAMEDBG
                (">>VPU_DEC: Packed Frame set with negative display idx \n");
            vpu_dec->allow_parallelization = FALSE;
            return VPU_DEC_CONTINUE;
        }
...
=============================================================================*/
GstFlowReturn
mfw_gst_vpu_dec_thread_get_output (MfwGstVPU_Dec * vpu_dec,
                                   gboolean fContinue)
{
...
mfw_gst_vpu_dec_event_thread (MfwGstVPU_Dec * vpu_dec)
{
    ...
    while (!bExit) {
            ...
            if (mfw_gst_vpudec_release_buff (vpu_dec) != GST_FLOW_OK) {
                ...
                if (vpu_dec->retval == RETCODE_SUCCESS) {
                    ...
                    if ((vpu_thread->retval != VPU_DEC_DONE) &&
                        ...
                        vpu_thread->retval =
                            vpu_thread_check_output (vpu_dec);
                    }
        ...
        case VPU_DEC_WAIT_EVENT:
            //GST_DEBUG(">>VPU_DEC: Event thread VPU_DEC_WAIT_EVENT getoutputpending=%d \n",vpu_thread->getoutput_pending);
