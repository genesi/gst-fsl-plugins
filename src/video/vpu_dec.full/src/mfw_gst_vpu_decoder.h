...
#define GST_VPU_QOS_EVENT_HANDLE(skipmode,diff) do {				            \
  	...
  	if ((micro_diff + VPU_DELAY_TIME_L1) > 0) {                                 \
    	if (micro_diff>VPU_DELAY_TIME_L3) {                                     \
        	skipmode = SKIP_BP;								                    \
            GST_WARNING ("The time of decoding is %d away the system time,"     \
                "drop B&P frames.\n", micro_diff);     				            \
            break;                                                              \
        }                                                                       \
    	else if (micro_diff>VPU_DELAY_TIME_L2) {                                \
        	skipmode = SKIP_L2;								                    \
            GST_DEBUG ("The time of decoding is %d away the system time,"       \
                "Enable L2 strategy\n", micro_diff);     				        \
            break;                                                              \
        } else {                                                                \
        	skipmode = SKIP_B;								                    \
            GST_INFO ("The time of decoding is %d away the system time,"        \
                "drop B frames\n", micro_diff);     				            \
            break;                                                              \
        }                                                                       \
    }                                                                           \
...
typedef struct _MfwGstVPU_Dec 
{
    ...
    gboolean force_nondr;       // Flag to force out of direct rendering
