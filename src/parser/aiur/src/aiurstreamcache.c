...
GstAiurStreamCache * 
gst_aiur_stream_cache_new(guint64 threshold_max, guint64 threshold_pre, void * context)
{
    GstAiurStreamCache * cache  = gst_mini_object_new(GST_TYPE_AIURSTREAMCACHE);
...
void 
gst_aiur_stream_cache_set_segment(GstAiurStreamCache * cache, guint64 start, guint64 stop)
{
    if (cache){
        g_mutex_lock(cache->mutex);
...
void 
gst_aiur_stream_cache_add_buffer(GstAiurStreamCache * cache, GstBuffer * buffer)
{
    guint64 size;
    gint trycnt = 0;
    ...
    if (cache->threshold_max){
        ...
        while ((gst_adapter_available(cache->adapter)>cache->threshold_max)&&(cache->closed==FALSE)){
		    if (((++trycnt) & 0x1f)==0x0) {
                GST_WARNING("wait push try %d SIZE %d %lld\n", trycnt, gst_adapter_available(cache->adapter), cache->threshold_max);
            }
...
tryseek:    
    ...
    if (addr<cache->start){/* left */
        ...
        cache->ignore_size = addr-cache->start-gst_adapter_available(cache->adapter);
...
trysendseek:
    ...
    cache->eos=FALSE;
    g_mutex_unlock(cache->mutex);
    ret =  gst_pad_push_event (cache->pad, gst_event_new_seek ((gdouble)1, GST_FORMAT_BYTES, GST_SEEK_FLAG_FLUSH, GST_SEEK_TYPE_SET, (gint64)addr,
...
gint64 
gst_aiur_stream_cache_read(GstAiurStreamCache * cache, guint64 size, char * buffer)
{
    gint64 readsize = -1;
    gint retrycnt=0;
    if (cache==NULL){
...
try_read:
    ...
    if ((cache->threshold_max) && (cache->threshold_max<size+cache->threshold_pre)){
        cache->threshold_max = size+cache->threshold_pre;
...
AiurLocalCacheLine * 
gst_aiur_local_cacheline_fill(AiurLocalCache * cache, AiurLocalCacheLine * line, guint64 address){
    GstFlowReturn ret;

	address &= cache->address_mask;
    ret = gst_pad_pull_range (cache->pad, address, cache->cacheline_size, &line->gstbuf);
	if (ret==GST_FLOW_OK){
...
gint 
gst_aiur_local_cache_read(AiurLocalCache * cache,  guint64 address, gint size, char * buf)
{
	...
	do{
        if ((line = gst_aiur_local_cacheline_find(cache, (address & cache->address_mask)))==NULL){
