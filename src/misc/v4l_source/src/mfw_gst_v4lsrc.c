 ...
 *
 * Portability:    This code is written for Linux OS, Gstreamer,
 *                  and the OV7690 camera from OmniVision
 */
...
/* enum for supported ov7690 camera modes
 *
 * TODO this is hack - adding a cap means affecting several places:
 *  the get_hardcoded_caps function, the anonymous enum of modes, get_mode_for_caps, and prep_capture_mode.
 *  if we (Genesi) were planning on maintaining this, we'd have made it more general...
 */
enum {
    MFW_V4L_MODE_MIN        = -1,
    MFW_V4L_MODE_VGA        = 0,
...
/* Capture mode macros */
#define IS_MODE_VGA(w, h) (((w) == 640) && ((h) == 480))
#define IS_MODE_QVGA(w, h) (((w) == 320) && ((h) == 240))

/* Check if valid mode */
#define IS_VALID_MODE(m) (((m) > MFW_V4L_MODE_MIN) && ((m) < MFW_V4L_MODE_MAX))

/* Weird version printing macro */
...
static GstCaps *mfw_gst_v4lsrc_get_hardcoded_caps(void);
...
static gboolean mfw_gst_v4lsrc_prep_capture_mode (MFWGstV4LSrc *v4l_src, int mode);
...
mfw_gst_v4lsrc_stop_capturing (MFWGstV4LSrc * v4l_src)
{
    ...
    GST_DEBUG ("telling V4L driver to stop capturing");
...
mfw_gst_v4lsrc_capture_setup (MFWGstV4LSrc * v4l_src)
{
    ...
    GST_DEBUG ("setting up V4L driver for video capture");
    ...
    GST_DEBUG ("called VIDIOC_S_PARM with timeperframe %d/%d",
                                parm.parm.capture.timeperframe.numerator,
                                parm.parm.capture.timeperframe.denominator);
        ...
        const GValue * value, GParamSpec * pspec)
{
    ...
    switch (prop_id) {
        ...
        case MFW_V4L_SRC_CAPTURE_MODE:
            mfw_gst_v4lsrc_prep_capture_mode (v4l_src, g_value_get_int (value));
...
/*
 * mfw_gst_v4lsrc_get_mode_for_caps - selects the best camera capture mode for requested caps
 * 
 * @caps:   downstream caps
 *
 * returns a capture mode for v4lsrc
 *
 * TODO this is hack - adding a cap means affecting several places:
 *  the get_hardcoded_caps function, the anonymous enum of modes, get_mode_for_caps, and prep_capture_mode.
 *  if we (Genesi) were planning on maintaining this, we'd have made it more general...
 */
static int
mfw_gst_v4lsrc_get_mode_for_caps (GstCaps *caps)
{
    GstStructure *caps_structure;
    gint width;
    gint height;
    int mode = MFW_V4L_MODE_UNDEFINED;

    caps_structure = gst_caps_get_structure(caps, 0); /* assumes only one caps */
    gst_structure_get_int(caps_structure, "width", &width);
    gst_structure_get_int(caps_structure, "height", &height);

    /*
        right now, our code does a 1-to-1 mapping of modes and accepted caps
        these are classified by the capture dimensions
    */
    /* TODO needs to not be hard coded */
    if      (width == 640 && height == 480)     mode = MFW_V4L_MODE_VGA;
    else if (width == 320 && height == 240)     mode = MFW_V4L_MODE_QVGA;
    else if (width == 352 && height == 288)     mode = MFW_V4L_MODE_CIF;
    else if (width == 176 && height == 144)     mode = MFW_V4L_MODE_QCIF;
    else                                        mode = MFW_V4L_MODE_UNDEFINED;

    GST_DEBUG ("mode %d chosen for format %dx%d",
                mode, width, height);
    return mode;
}

/*!
 * mfw_gst_v4lsrc_prep_capture_mode - helper function called by 
 *  mfw_gst_v4lsrc_set_caps to hardcode capture parameters, based on mode
 *
 * @v4l_src: pointer to our V4L source
 * @mode: the capture mode to configure the device for
 *
 * return whether setup was successful
 *
 * NOTE: This appears to be the only code that sets our src before 
 *  it is used in turn to set the device.
 *
 * TODO this is hack - adding a cap means affecting several places:
 *  the get_hardcoded_caps function, the anonymous enum of modes, get_mode_for_caps, and prep_capture_mode.
 *  if we (Genesi) were planning on maintaining this, we'd have made it more general...
 */
static gboolean
mfw_gst_v4lsrc_prep_capture_mode (MFWGstV4LSrc *v4l_src, int mode)
{
    GstBaseSrc *src = GST_BASE_SRC (v4l_src);

    /* TODO this should be read from the camera!*/           /*VGA      QVGA    CIF     QCIF*/
    static const gint mode_widths[MFW_V4L_MODE_MAX]         = {640,     320,    352,    176};
    static const gint mode_heights[MFW_V4L_MODE_MAX]        = {480,     240,    288,    144};
    static const gint mode_timeperframe_n[MFW_V4L_MODE_MAX] = {1,       1,      1,      1};
    static const gint mode_timeperframe_d[MFW_V4L_MODE_MAX] = {30,      30,     30,     30};

    gboolean was_capturing = v4l_src->start;

    GST_DEBUG ("preparing camera for capture mode %d", mode);

    if (! IS_VALID_MODE(mode)) {
            GST_ERROR ("cannot setup capture: invalid mode %d given", mode);
            return FALSE;
    }

    if (mode == v4l_src->capture_mode) {
            GST_DEBUG ("mode %d already set, doing nothing", mode);
            return TRUE; /* already set properly, nothing to be done */
    }

    if (was_capturing) { /* guessing I can't set caps while video is rolling through pipeline */
            GST_DEBUG("asked to set caps while capturing. Will restart");
            mfw_gst_v4lsrc_stop(src);
    }
    ...
    if (was_capturing) {
            GST_DEBUG("resuming capture with new caps");
            mfw_gst_v4lsrc_start(src);
    }
...
mfw_gst_v4lsrc_set_caps (GstBaseSrc * src, GstCaps * caps)
{
    ...
    mode_for_caps = mfw_gst_v4lsrc_get_mode_for_caps (caps);

    if (!mfw_gst_v4lsrc_prep_capture_mode (v4l_src, mode_for_caps)) {
        GST_ERROR ("unable to set caps!");
        return FALSE;
    }
...
mfw_gst_v4lsrc_stop (GstBaseSrc * src)
{
    ...
    GST_DEBUG ("stopping video capture");
    ...
    GST_DEBUG ("Successfully stopped capture");
    return TRUE;
}
 ...
 */
static GstCaps *
mfw_gst_v4lsrc_get_hardcoded_caps()
{
    GstCaps *capslist;
#ifndef MX51
...
mfw_gst_v4lsrc_get_caps (GstBaseSrc * src)
{
    ...
    return mfw_gst_v4lsrc_get_hardcoded_caps(); /* TODO should query machine */
}
...
mfw_gst_v4lsrc_fixate (GstPad * pad, GstCaps * caps)
{
    ...
    GST_DEBUG("asked to fixate caps");
