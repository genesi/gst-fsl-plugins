...
mfw_gst_v4lsink_buffer_finalize(MFWGstV4LSinkBuffer *v4lsink_buffer_released)
{
    ...
    case BUF_STATE_ALLOCATED:
        ...
        if (!(v4l_info->frame_dropped & 0x3f)){
            GST_ERROR(">>V4L_SINK: %d dropped while %d showed!\n", v4l_info->frame_dropped, v4l_info->qbuff_count);
        }
    case BUF_STATE_SHOWED:
        ...
        if (GST_BUFFER_FLAG_IS_SET(v4lsink_buffer_released, GST_BUFFER_FLAG_LAST)){
            ...
            if (!IS_RESERVED_HWBUFFER_FULL(v4l_info)){
                ...
                GST_DEBUG("Push to reserved buffer pool:%d\n", g_slist_length((v4l_info)->reservedhwbuffer_list) );
            }else{
                v4l_info->free_pool = g_slist_append(v4l_info->free_pool, v4lsink_buffer_released);
                GST_DEBUG("Push to free buffer pool:%d\n", g_slist_length((v4l_info)->free_pool));
            ...
            GST_DEBUG(RED_STR("%s: free software buffer\n",__FUNCTION__));
    ...
    case BUF_STATE_FREE:
        ...
        if (GST_BUFFER_DATA(v4lsink_buffer_released)){
            if (GST_BUFFER_FLAG_IS_SET(v4lsink_buffer_released, GST_BUFFER_FLAG_LAST)){
	            ...
	            v4l_info->all_buffer_pool[v4lsink_buffer_released->v4l_buf.index] = NULL;
	            v4l_info->querybuf_index--;
	            GST_DEBUG(">>V4L_SINK: Buffer %d is freed.\n", v4lsink_buffer_released->v4l_buf.index);

            }else{
                GST_DEBUG(">>V4L_SINK: Software Buffer %p is freed.\n", GST_BUFFER_DATA(v4lsink_buffer_released));
                g_free(GST_BUFFER_DATA(v4lsink_buffer_released));

            }
        }
        {
            gint index = G_N_ELEMENTS(v4lsink_buffer_released->buffer._gst_reserved)-1;
            if (GST_IS_BUFFER_META(v4lsink_buffer_released->buffer._gst_reserved[index])) {
                GST_INFO("Free buffer meta context");
                gst_buffer_meta_free(v4lsink_buffer_released->buffer._gst_reserved[index]);
            }
        }
        if (v4l_info){
...
IMPORTANT NOTES:    None
=============================================================================*/
MFWGstV4LSinkBuffer *mfw_gst_v4l2_new_swbuffer(MFW_GST_V4LSINK_INFO_T * v4l_info)
{
    ...
    switch (v4l_info->outformat) {
        ...
        case V4L2_PIX_FMT_NV12:
            buf_size = (v4l_info->width+v4l_info->cr_left_bypixel+v4l_info->cr_right_bypixel)*
                            (v4l_info->height+v4l_info->cr_left_bypixel+v4l_info->cr_right_bypixel)*3/2;
                
            break;
        case V4L2_PIX_FMT_RGB24:
            buf_size = (v4l_info->width+v4l_info->cr_left_bypixel+v4l_info->cr_right_bypixel)*
                            (v4l_info->height+v4l_info->cr_left_bypixel+v4l_info->cr_right_bypixel)*3;

            break;
        case V4L2_PIX_FMT_RGB32:
            buf_size = (v4l_info->width+v4l_info->cr_left_bypixel+v4l_info->cr_right_bypixel)*
                            (v4l_info->height+v4l_info->cr_left_bypixel+v4l_info->cr_right_bypixel)*4;

            break;
            
        default:
            GST_ERROR("Unsupport format:%x\n", v4l_info->outformat);        
            break;
...
MFWGstV4LSinkBuffer *mfw_gst_v4l2_new_buffer(MFW_GST_V4LSINK_INFO_T* v4l_info)
{
    ...
    struct v4l2_buffer *v4lbuf = NULL;
    {
        ...
        if ((IS_RESERVED_HWBUFFER_FULL(v4l_info)) && (tmp = v4l_info->free_pool)){
                ...
                g_mutex_unlock(v4l_info->pool_lock);
                GST_DEBUG("Assign a buffer from queue, available :%d.\n", 
				           g_slist_length(v4l_info->free_pool));
                return v4lsink_buffer;
        }
        ...
        while ((loopcount++)<BUFFER_NEW_RETRY_MAX){
            ...
            if (tmp = v4l_info->free_pool){
                ...
                v4l_info->free_pool = g_slist_delete_link(v4l_info->free_pool, tmp);
                g_mutex_unlock(v4l_info->pool_lock);
                GST_DEBUG("After DQ, assign a hw buffer from queue,queued:%d, left:%d.\n", v4l_info->v4lqueued,
                    g_slist_length(v4l_info->free_pool));
                return v4lsink_buffer;
            }
            if (ret<0) {
        ...
        GST_WARNING(">>V4L_SINK: Try new buffer failed, ret %d %s queued %d\n", 
...
MFWGstV4LSinkBuffer *mfw_gst_v4l2_new_hwbuffer(MFW_GST_V4LSINK_INFO_T* v4l_info)
{
    ...
    if (ioctl(v4l_info->v4l_id, VIDIOC_QUERYBUF, v4lbuf) < 0) {
        GST_ERROR(">>V4L_SINK: VIDIOC_QUERYBUF failed %d, device id:%d,  err:%s\n", v4l_info->querybuf_index,
            v4l_info->v4l_id, strerror(errno));
    ...
    mmap(NULL, v4lbuf->length,
         ...
         v4l_info->v4l_id, v4lbuf->m.offset);
...
gboolean mfw_gst_v4l2_dq_buffer(MFW_GST_V4LSINK_INFO_T *v4l_info)
{
    struct v4l2_buffer v4l2buf;
    gboolean ret = FALSE;
    ...
    if(!((ioctl(v4l_info->v4l_id, VIDIOC_DQBUF, &v4l2buf))< 0)) {
        ...
        v4lsinkbuffer = (MFWGstV4LSinkBuffer  *)(v4l_info->all_buffer_pool[v4l2buf.index]);
        if ((v4lsinkbuffer) && (v4lsinkbuffer->bufstate == BUF_STATE_SHOWING)){
            ...
            GST_INFO("DQ buffer state:%d\n", v4lsinkbuffer->bufstate);
        }
        ret = TRUE;
    } 
    g_mutex_unlock(v4l_info->pool_lock);
...
gboolean mfw_gst_v4l2_queue_buffer(MFW_GST_V4LSINK_INFO_T *v4l_info, 
                                    struct v4l2_buffer *v4l2buf)
{
    gboolean ret = FALSE;
    return ret;
...
GstFlowReturn mfw_gst_v4l2_buffer_init(MFW_GST_V4LSINK_INFO_T *v4l_info) 
{
        ...
        for (i=0;i<totalbuffernum;i++)
        {
            ...
            if (v4lsink_buffer)
            {
                    ...
                    (v4lsink_buffer->bufstate == BUF_STATE_SHOWING))
                {
                    if (v4lsink_buffer->bufstate == BUF_STATE_IDLE)
                    {
                        v4l_info->free_pool = g_slist_remove(v4l_info->free_pool, v4lsink_buffer);
                        v4l_info->reservedhwbuffer_list =
                            g_slist_remove(v4l_info->reservedhwbuffer_list, v4lsink_buffer);
                    }
                    v4lsink_buffer->bufstate = BUF_STATE_FREE;
                    gst_buffer_unref(GST_BUFFER_CAST(v4lsink_buffer));
                } else {
