...
mfw_gst_x11_handle_xevents (void *context)
{
    ...
    while (XCheckWindowEvent (gstXInfo->xcontext->disp,
            gstXInfo->xwindow->win, ExposureMask | StructureNotifyMask
      ...
      switch (e.type) {
        ...
        gst_navigation_send_mouse_event (GST_NAVIGATION (gstXInfo->parent),
        ...
        if (keysym != NoSymbol) {
          ...
          g_mutex_unlock (gstXInfo->x_lock);
          gst_navigation_send_key_event (GST_NAVIGATION (gstXInfo->parent),
                                ...
                                gint disp_width, gint disp_height)
{
    ...
    GST_INFO(BLUE_STR("Set Black Borders (lw,lh,rw,rh) = (%d, %d, %d, %d).\n",lw, lh, rw, rh));
                          ...
                          gint width, gint height)
{
    ...
    GST_INFO("set xwindow color :0x%08x in (%dx%d)\n",
        color,
        width, height);

    GST_INFO("Borders (lw,lh,width, height) = (%d, %d, %d, %d).\n",lw, lh, 
        width, height);
...
{
    GST_INFO(RED_STR("Clear xwindow by set black from (%d,%d) in screen (%d x %d)\n",
        lw, lh, width, height));
                                ...
                                gint disp_width, gint disp_height)
{
    ...
    mfw_gst_x11_set_black_borders(gstXInfo,
        lw,lh,rw,rh,width,height,disp_width,disp_height);
...
gboolean
mfw_gst_x11_xwindow_decorate (GstXInfo * gstXInfo,
    GstXWindow * window)
{
...
mfw_gst_x11_get_geometry(GstXInfo *gstXInfo)
{
    ...
    if(gstXInfo->xcontext == NULL
            || gstXInfo->xwindow->win == 0
            || gstXInfo->xwindow->gc == 0)
        return FALSE;


    g_mutex_lock (gstXInfo->x_lock);

    XGetWindowAttributes (gstXInfo->xcontext->disp, gstXInfo->xwindow->win, &attr);
    XGetWindowAttributes (gstXInfo->xcontext->disp, attr.root, &root_attr);
    XTranslateCoordinates(gstXInfo->xcontext->disp, gstXInfo->xwindow->win, attr.root,
            0, 0, &x, &y, &w);
    ...
    gint width, gint height)
{
    ...
    GST_DEBUG("%s: create sink own window.\n", __FUNCTION__);
    ...
    * ConfigureNotify. This takes away flickering of video when resizing. */
    ...
    gst_x_overlay_got_xwindow_id (GST_X_OVERLAY (gstXInfo->parent), xwindow->win);
...
void mfw_gst_xcontext_free(GstXInfo * gstXInfo)
{
...
void mfw_gst_xwindow_create(GstXInfo * gstXInfo, XID xwindow_id)
{
    ...
    xwindow->gc = XCreateGC (gstXInfo->xcontext->disp, xwindow->win,
            0, NULL);
