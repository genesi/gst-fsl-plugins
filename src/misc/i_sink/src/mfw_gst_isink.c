...
static ISinkFmt isink_yuv_fmts[] = 	{
    ...
    {GST_MAKE_FOURCC('Y', '4', '4', '4'),GST_MAKE_FOURCC('Y', '4', '4', '4'), 24},
    {FOURCC_NONE, 0}
};
...
#if 0
void  isink_free_external_frame(GstMfwBuffer * buf)
{
...
#else
static
void  isink_free_external_frame(gpointer data)
{
...
static
void  isink_free_hwbuffer(gpointer data)
{
    GstBufferMeta * meta;
    if (meta = (GstBufferMeta*)data){
				     ...
				     GstBuffer ** buf)
{
    ...
    if (G_UNLIKELY(isink->init == FALSE)) 
    {
        ...
        icfg->srcfmt.croprect.width += (icfg->srcfmt.croprect.win.left +right);
        icfg->srcfmt.croprect.height +=  (icfg->srcfmt.croprect.win.top + bottom);
        ...
        if (isink->need_use_ex_buffer){
           ...
           if (icfg->requestBufferNum==0){
                ...
                icfg->srcfmt.croprect.width = 
                    ((icfg->origsrcfmt.croprect.win.right-icfg->origsrcfmt.croprect.win.left)+63)/64*64;
                icfg->srcfmt.croprect.win.left = 0;
                icfg->srcfmt.croprect.win.right = (icfg->origsrcfmt.croprect.win.right-icfg->origsrcfmt.croprect.win.left);
                
                icfg->srcfmt.croprect.height = 
                    ((icfg->origsrcfmt.croprect.win.bottom-icfg->origsrcfmt.croprect.win.top)+15)/16*16;
                icfg->srcfmt.croprect.win.top = 0;
                icfg->srcfmt.croprect.win.bottom = (icfg->origsrcfmt.croprect.win.bottom-icfg->origsrcfmt.croprect.win.top);
            }
            int i;
            ISinkCallBack icallback;
            ISinkFrameAllocInfo * bufinfo = g_malloc(sizeof(ISinkFrameAllocInfo)+sizeof(ISinkFrame *)*icfg->requestBufferNum);
...
#if 0    
        ibuf = gst_mfw_buffer_new_and_alloc(isink->icfg.framebufsize, GST_MFW_BUFFER_FLAG_DMABLE);
#else
            gint index;
            GstBufferMeta *bufmeta;
            unsigned int *vaddr, *paddr;    
            void * handle;
            if (!(handle = mfw_new_hw_buffer(isink->icfg.framebufsize, &paddr, &vaddr, 0))) {
                GST_ERROR(">>I_SINK: Could not allocate hardware buffer\n");
                *buf =  NULL;
                return GST_FLOW_ERROR;
            }
            ibuf = gst_buffer_new();
            GST_BUFFER_SIZE(ibuf) = isink->icfg.framebufsize;
        ...
        default:
            {
                ...
                ocfg = &isink->ocfg[idx];
                idx = prop_id-idx*ISINK_PROP_DISP_LENGTH;

                if (idx!=ISINK_PROP_DISP_NAME_0){
...
int mfw_gst_isink_get_default_width(MfwGstISink *isink, int devid, int mode)
{
    int value = 0;
    MfwGstISinkClass * class = isink->class;
    VideoDeviceDesc * vd_desc = class->vd_desc;
...
int mfw_gst_isink_get_default_height(MfwGstISink *isink, int devid, int mode)
{
    int value = 0;
    MfwGstISinkClass * class = isink->class;
    VideoDeviceDesc * vd_desc = class->vd_desc;
...
mfw_gst_create_display(MfwGstISink *isink, int idx)
{
    OutCfg * ocfg = &isink->ocfg[idx];
        ...
        if (RECT_WIDTH(&ocfg->desfmt.rect)==0)
            ocfg->desfmt.rect.right+=mfw_gst_isink_get_default_width(isink,ocfg->devid,ocfg->mode);
        if (RECT_HEIGHT(&ocfg->desfmt.rect)==0)
            ocfg->desfmt.rect.bottom+=mfw_gst_isink_get_default_height(isink,ocfg->devid,ocfg->mode);
        
        ocfg->vshandle = createVideoSurface(ocfg->devid, ocfg->mode, &isink->icfg.srcfmt, &ocfg->desfmt);
...
static void mfw_gst_isink_copy_frame_i420_y(char * dest, SourceFmt * destfmt, char * src, SourceFmt * srcfmt)
{
    int i, width, height;
    dest+=(destfmt->croprect.win.left+destfmt->croprect.win.top*destfmt->croprect.width);
    src+=(srcfmt->croprect.win.left+srcfmt->croprect.win.top*srcfmt->croprect.width);
...
static void mfw_gst_isink_copy_frame_i420_uv(char * dest, SourceFmt * destfmt, char * src, SourceFmt * srcfmt)
{
    int i, width, height;
    dest+=(destfmt->croprect.win.left/2+destfmt->croprect.win.top*destfmt->croprect.width/4);
    src+=(srcfmt->croprect.win.left/2+srcfmt->croprect.win.top*srcfmt->croprect.width/4);
                                                ...
                                                GstBuffer * gstbuf)
{
    ...
    if (isink->need_render_notify){
        ...
        if ((isink->need_use_ex_buffer) && (isink->need_force_copy)){
            ...
            src+=isink->icfg.origsrcfmt.croprect.width*isink->icfg.origsrcfmt.croprect.height;
            mfw_gst_isink_copy_frame_i420_uv(iframe->vaddr[1], &isink->icfg.srcfmt, 
                src, &isink->icfg.origsrcfmt);
            src+=isink->icfg.origsrcfmt.croprect.width*isink->icfg.origsrcfmt.croprect.height/4;
    ...
    if (gstbuf->_gst_reserved[index]){
        ...
        if (GST_MFWBUFFER_SUBBUF(gstbuf)){
            SubDesc * subdesc = (SubDesc *)GST_BUFFER_DATA(GST_MFWBUFFER_SUBBUF(gstbuf));
...
mfw_gst_isink_class_init(MfwGstISinkClass * klass)
{
    ...
    g_object_class_install_property(gobject_class, ISINK_PROP_USE_EXTERNAL_BUFFER,
                    g_param_spec_boolean("use-external-buffer",
                             "use-external-buffer",
                             "set to make isink use external buffer",
                             FALSE,
                             G_PARAM_WRITABLE));
    g_object_class_install_property(gobject_class, ISINK_PROP_RENDER_NOTIFY,
                g_param_spec_boolean("render-notify",
                         "render-notify",
                         "set to make isink callback when render",
                         FALSE,
                         G_PARAM_WRITABLE));
    ...
    g_object_class_install_property(gobject_class, prop,
                g_param_spec_int(prop_name,
                         prop_name,
                         "get/set the rotation of the output",
                         0, 7, 0,
                         G_PARAM_READWRITE));
    g_free(prop_name);
    prop++;
    ...
    while (fmt->fourcc!=FOURCC_NONE) {
    	gst_caps_append_structure(capslist,
            gst_structure_new("video/x-raw-yuv",
                ...
                "width_align", G_TYPE_INT, 64,
                "height_align", G_TYPE_INT, 64,
                 NULL));
        fmt++;
    }
