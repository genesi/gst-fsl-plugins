...
mfw_gst_xisink_get_geometry(MfwGstISink *isink)
{
    ...
    if ( (isink->ocfg[0].desfmt.rect.right-isink->ocfg[0].desfmt.rect.left) != width
    || (isink->ocfg[0].desfmt.rect.bottom-isink->ocfg[0].desfmt.rect.top) != height) {
...
gboolean mfw_gst_xisink_set_color (MfwGstISink *isink, Rect * setrect)
{
    GstXInfo *gstXInfo = isink->gstXInfo;
#if 1
    ...
    mfw_gst_x11_set_color_borders(gstXInfo, isink->colorkey,lw, lh, rw, rh, 
        width, height, (setrect->right-setrect->left), (setrect->bottom-setrect->top));
#endif
...
IMPORTANT NOTES:    None
=============================================================================*/
gboolean mfw_gst_xisink_clear_color (MfwGstISink *isink, Rect * newrect)
{
    GstXInfo *gstXInfo = isink->gstXInfo;
#if 1    
            ...
            if ((mfw_gst_x11_handle_xevents (isink->gstXInfo))|| (isink->setXid)  ) {
                ...
                if (param){
                    ...
                    if (isink->ocfg[0].vshandle){
                        ...
                        configVideoSurface(isink->ocfg[0].vshandle, CONFIG_MASTER_PARAMETER, &config);
